# src/scrapers/cercledemartillac.py
"""
Cercle de Martillac scraper — Playwright required.
PrestaShop site returns 403 to plain requests (bot detection).
...
PRICE_SELECTORS = ["span.price", "span[itemprop='price']", ".product-price", ".prix"]


class CercleDeMartillacScraper(BaseScraper):
    retailer = "cercledemartillac"
            ...
            try:
                for vintage in vintages:
                    try:
                        ...
                        logger.warning(
                            f"cercledemartillac: failed {product.estate_name} {vintage}: {e}"
                        )
            finally:
                browser.close()
    ...
    def _scrape_page(self, context, url: str, vintage: int, product) -> ScrapeResult | None:
        ...
        try:
            ...
            if response and response.status in (404, 403):
                logger.info(f"cercledemartillac: {response.status} at {url} — skipping")
                return None
            ...
            if not raw_price:
                logger.warning(f"cercledemartillac: no price found at {url}")
                return None

            try:
                price_amount, currency = parse_price(raw_price)
            except ValueError as e:
                logger.warning(f"cercledemartillac: could not parse '{raw_price}' at {url}: {e}")
                return None
            ...
            logger.info(
                f"cercledemartillac: {product.estate_name} {vintage} — "
