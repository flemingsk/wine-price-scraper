# src/scrapers/wineandco.py
"""
Wine&Co scraper — requires Playwright (JS-rendered prices).
Page explicitly says "activate JavaScript" to non-JS clients.
Price selector: .product-price or span[itemprop='price']
"""
import logging
...
class WineandcoScraper(BaseScraper):
    retailer = "wineandco"
            ...
            try:
                for vintage in vintages:
                    try:
                        ...
                        if not url:
                            logger.warning(f"wineandco: no URL for {product.estate_name} {vintage}")
                            continue
                        ...
                        logger.warning(f"wineandco: failed {product.estate_name} {vintage}: {e}")
            finally:
                browser.close()
    ...
    def _scrape_page(self, context, url: str, vintage: int, product) -> ScrapeResult | None:
        ...
        try:
            ...
            if not raw_price:
                logger.warning(f"wineandco: no price found at {url}")
                return None

            try:
                price_amount, currency = parse_price(raw_price)
            except ValueError as e:
                logger.warning(f"wineandco: could not parse '{raw_price}' at {url}: {e}")
                return None
            ...
            logger.info(
                f"wineandco: {product.estate_name} {vintage} — "
