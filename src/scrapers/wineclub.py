# src/scrapers/wineclub.py
"""
LeWineClub scraper — requires Playwright (JS-rendered prices).
Price selector: span.normal-price or span[itemprop='price']
"""
import logging
...
class WineclubScraper(BaseScraper):
    retailer = "wineclub"
            ...
            try:
                for vintage in vintages:
                    try:
                        ...
                        if not url:
                            logger.warning(f"wineclub: no URL for {product.estate_name} {vintage}")
                            continue
                        ...
                        logger.warning(f"wineclub: failed {product.estate_name} {vintage}: {e}")
            finally:
                browser.close()
    ...
    def _scrape_page(self, context, url: str, vintage: int, product) -> ScrapeResult | None:
        ...
        try:
            ...
            if not raw_price:
                logger.warning(f"wineclub: no price found at {url}")
                return None

            try:
                price_amount, currency = parse_price(raw_price)
            except ValueError as e:
                logger.warning(f"wineclub: could not parse '{raw_price}' at {url}: {e}")
                return None
            ...
            logger.info(
                f"wineclub: {product.estate_name} {vintage} — "
